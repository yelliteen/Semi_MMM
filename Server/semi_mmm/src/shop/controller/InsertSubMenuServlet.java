...
/**
 * Servlet implementation class InsertSubMenuServlet
 */
@WebServlet(name = "InsertSubMenu", urlPatterns = { "/insertSubMenu" })
public class InsertSubMenuServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public InsertSubMenuServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//3. 비지니스 로직
		int result = new ShopService().insertSubMenu(po);	
		//4. 결과처리
		RequestDispatcher rd = request.getRequestDispatcher("/WEB-INF/views/common/msg.jsp");
		if(result>0) {
			request.setAttribute("msg", "하위메뉴 추가 성공");
		}else {
			request.setAttribute("msg", "하위메뉴 추가 실패");
		}
